...
void Big8wPrint(big8w* bignum)
{
	...
	printf("0x %08x", bignum->word[i]);

	if(i--) // i > 0
		for (; i>=0; i--) 
			printf(" %08x", bignum->word[i]);
		
	printf("\n");
}

void U8StringToBig8w(uint8_t* string, big8w* x)
{
	int i;
	for (i = 1; i < BIGNUMBER_SIZE_8WORD + 1; i++) 
		x->word[BIGNUMBER_SIZE_8WORD - i] = GETU32(string + 4 * (i - 1));
}
 ...
 * 
*/
void Big8wHighestbit(big8w* bignum, uint32_t *numindex, uint32_t *bitindex)
{
	uint32_t elem;
	int i = BIGNUMBER_SIZE_8WORD - 1;
...
big8w Big16wmod8w(big16w bignum16w, big8w p)
{
	...
	Big8wHighestbit(&p, &plen, &pbit);
	pbitlen = (plen << 5) + pbit;

	while (bignum16w.length > 7){
		gap = bignum16w.length * 32 + Big16wHighestbit(bignum16w) - pbitlen; 
...
/**
 * 
 * @brief get reverse of bignum under N; implemented with Stein algorithm.  Calls: Big8wRightShift, PlusAndRightShiftOne, Big8wEqual, Big8wMinusMod
 * 
 * @param bignum a big number
...
{
	int numindex = BIGNUMBER_SIZE_8WORD - 1, i;
...
/**
 * 
 * @brief generate random big number based on crypto_rng_read, bad version for test. 
 * TODO: replace
 * 
 * @param null 
 * 
